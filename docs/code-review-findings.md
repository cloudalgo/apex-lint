...
### H7 ✅ `ApexSOQLInjection` missed the canonical escaped-quote pattern — **FIXED** (security)
...
**Note on "why these rules find nothing":** `ApexSOQLInjection` (critical) is taint-gated — it fires only when a recognized user-controlled source (VF params, REST params/body, cookies, current URL) reaches `Database.query()` unsanitized. The bundled test repos (NPSP, EDA, dreamhouse, fflib, apex-recipes, …) are mature and security-reviewed, so they legitimately contain no such path → 0 is correct, not broken (the sample fixture proves the rule works). A real **coverage gap** remains: the taint sources do **not** include method parameters — notably `@AuraEnabled` (LWC) and `webservice` Apex params, the modern attack surface — so param→concat→query is not flagged by the critical rule (the broad `DatabaseQueryWithVariable` catches those generically at `high`). Worth adding `@AuraEnabled`/`webservice` params as taint sources in a follow-up.

### H4 ◽ Taint engine re-walks every method 10+ times per file
`packages/apex-core/src/rules/security.ts`
The 4 taint rules (`ApexSOQLInjection`, `ApexOpenRedirect`, `ApexSSRF`, `ApexXSSFromURLParam`) each fire on every `MethodDeclarationContext`; `buildTaintedVars` runs 5 fixed-point passes, each doing `walk(method)` **and** `textOf(method)` (full subtree concat), recomputed identically every pass.
**Fix:** hoist `textOf(method)` out of the loop; compute `buildTaintedVars` once per method and share across the 4 rules.

### H5 ◽ Duplicated test-detection helpers have drifted
`classHasIsTest` / `isInsideTestClass` / `hasAnnotationOn` are copy-pasted across `crud`, `hardcoded`, `performance`, `style`, `security`, `async`, `nre`. They already disagree: `style.ts classIsTest` checks only the outer class, while `security.ts isInsideTestContext` walks all ancestors — so rules differ on what counts as test code.
**Fix:** extract one `ast/apex-helpers.ts` (`isTestClass`, `isInsideTestClass`, `hasAnnotation`, `getImplementsText`).
